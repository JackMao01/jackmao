(() => {
 ...
 let secondary=false;[...menu.children].forEach(el=>{if(el.tagName==='SPAN'&&el.textContent.trim()==='More'){el.textContent='Résumé & social';secondary=true}if(secondary&&el.tagName==='A')el.classList.add('mpm-secondary')});
 ...
 const open=()=>{menu.inert=false;menu.removeAttribute('aria-hidden');menu.classList.add('is-open');bar.classList.add('is-open');trigger.setAttribute('aria-expanded','true');menu.querySelector('button').focus({preventScroll:true})};
 ...
 addEventListener('pagehide',()=>close());close();
})();
